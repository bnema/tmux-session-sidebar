...
func (c Client) CaptureAttachedSidebarWidthBaseline(ctx context.Context, windowID string, paneID string, width string, options ports.SidebarResizeOptions) error {
	...
	if windowID == "" || paneID == "" {
		c.resizeDebug("resize-baseline-capture-skip", ports.LogField{Key: "reason", Value: "missing-target"}, ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "pane", Value: paneID}, ports.LogField{Key: "width", Value: width})
		return nil
	}
	c.resizeDebug("resize-baseline-capture-start", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "pane", Value: paneID}, ports.LogField{Key: "width", Value: width})
...
func (c Client) SyncAttachedSidebarWidth(ctx context.Context, windowID string, paneID string, width string, options ports.SidebarResizeOptions) error {
	...
	if windowID == "" || paneID == "" {
		c.resizeDebug("resize-sync-skip", ports.LogField{Key: "reason", Value: "missing-target"}, ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "pane", Value: paneID}, ports.LogField{Key: "width", Value: width})
		return nil
	}
	c.resizeDebug("resize-sync-start", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "pane", Value: paneID}, ports.LogField{Key: "width", Value: width})
	...
	c.resizeDebug("resize-sidebar-pane", ports.LogField{Key: "pane", Value: paneID}, ports.LogField{Key: "width", Value: width})
...
func (c Client) saveSidebarOpenWorkBaseline(ctx context.Context, windowID string, weights []sidebarWorkWeight) error {
	...
	for _, weight := range weights {
		if strings.TrimSpace(weight.RepresentativePaneID) == "" || weight.Weight <= 0 {
			c.resizeDebug("resize-baseline-clear", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "reason", Value: "invalid-weight"}, ports.LogField{Key: "weights", Value: formatSidebarWorkWeights(weights)})
	...
	if len(baseline.RepresentativePaneIDs) < 2 {
		c.resizeDebug("resize-baseline-clear", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "reason", Value: "too-few-work-groups"}, ports.LogField{Key: "weights", Value: formatSidebarWorkWeights(weights)})
		return c.clearWindowOptionValue(ctx, windowID, optionSidebarOpenWorkBaseline)
	}
	...
	c.resizeDebug("resize-baseline-save", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "representatives", Value: baseline.RepresentativePaneIDs}, ports.LogField{Key: "widths", Value: baseline.WorkWidths})
...
func (c Client) loadSidebarOpenWorkBaseline(ctx context.Context, windowID string) ([]sidebarWorkWeight, error) {
	...
	if err := json.Unmarshal([]byte(raw), &baseline); err != nil {
		c.resizeDebug("resize-baseline-invalid", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "reason", Value: "decode"}, ports.LogField{Key: "raw", Value: raw}, ports.LogField{Key: "error", Value: err.Error()})
		return nil, nil
	}
	if len(baseline.RepresentativePaneIDs) < 2 || len(baseline.RepresentativePaneIDs) != len(baseline.WorkWidths) {
		c.resizeDebug("resize-baseline-invalid", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "reason", Value: "shape"}, ports.LogField{Key: "representatives", Value: baseline.RepresentativePaneIDs}, ports.LogField{Key: "widths", Value: baseline.WorkWidths})
		return nil, nil
	}
	...
	for i, repID := range baseline.RepresentativePaneIDs {
		...
		if repID == "" || baseline.WorkWidths[i] <= 0 {
			c.resizeDebug("resize-baseline-invalid", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "reason", Value: "invalid-entry"}, ports.LogField{Key: "representatives", Value: baseline.RepresentativePaneIDs}, ports.LogField{Key: "widths", Value: baseline.WorkWidths})
			return nil, nil
		}
		weights = append(weights, sidebarWorkWeight{RepresentativePaneID: repID, Weight: baseline.WorkWidths[i]})
	}
	c.resizeDebug("resize-baseline-loaded", ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "representatives", Value: baseline.RepresentativePaneIDs}, ports.LogField{Key: "widths", Value: baseline.WorkWidths})
	return weights, nil
}
...
func (c Client) applySidebarWorkWeightsBestEffort(ctx context.Context, windowID string, sidebarPaneID string, weights []sidebarWorkWeight, requireSidebar bool) {
	...
	for i, weight := range weights {
		...
		if representativePaneID == "" || weight.Weight <= 0 {
			c.resizeDebug("resize-work-weights-skip", ports.LogField{Key: "reason", Value: "invalid-baseline-weight"}, ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "sidebar", Value: sidebarPaneID}, ports.LogField{Key: "weights", Value: formatSidebarWorkWeights(weights)})
			return
		}
		group, ok := groupByRep[representativePaneID]
		if !ok {
			c.resizeDebug("resize-work-weights-skip", ports.LogField{Key: "reason", Value: "missing-current-group"}, ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "sidebar", Value: sidebarPaneID}, ports.LogField{Key: "representative", Value: representativePaneID}, ports.LogField{Key: "weights", Value: formatSidebarWorkWeights(weights)}, ports.LogField{Key: "groups", Value: formatSidebarHorizontalGroups(groups)})
			return
		}
	...
	for _, targetWidth := range targetWidths {
		if targetWidth <= 0 {
			c.resizeDebug("resize-work-weights-skip", ports.LogField{Key: "reason", Value: "invalid-target-width"}, ports.LogField{Key: "window", Value: windowID}, ports.LogField{Key: "sidebar", Value: sidebarPaneID}, ports.LogField{Key: "weights", Value: formatSidebarWorkWeights(weights)}, ports.LogField{Key: "target_widths", Value: targetWidths})
			return
		}
	}

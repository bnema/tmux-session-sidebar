...
func TestRunUsesSignalAwareContext(t *testing.T) {
	...
	var gotRouterContext context.Context
	newRouter = func(ctx context.Context) app.Router {
		gotRouterContext = ctx
		return nil
	}
	...
	if gotRouterContext != gotContext {
		t.Fatalf("newRouter context = %v, want app.Run context %v", gotRouterContext, gotContext)
	}

...
main() {
  ...
  local sidebar_key previous_key quoted_runtime runtime_bin slot state_dir
  ...
  "$TMUX_BIN" run-shell -b "mkdir -p \"$state_dir\" && \"$runtime_bin\" daemon ensure >/dev/null 2>>\"$state_dir/errors.log\""
